...
module Opal
  module Nodes
    class CallNode < Base
      ...
      def recvr_is_const_named?(const_name)
        recvr && recvr.type == :const && begin
          const_base = recvr.children.first

          const_base && const_base.type == :cbase
        end && recvr.children[1] == const_name
      end
      ...
      def handle_special(&compile_default)
        special = SPECIALS[meth]
        if special && (!special[:const] || recvr_is_const_named?(special[:const]))
      ...
      add_special :require do |compile_default|
        ...
        compiler.require str unless str.nil?
        compile_default.call
      end
      ...
      add_special :require_relative do
        ...
        if arg.type == :str
          ...
          compiler.require Pathname(dir).join(arg.children[0]).cleanpath.to_s
        end
